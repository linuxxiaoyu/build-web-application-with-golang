...
## Socket基础知识
通过上面的介绍我们知道Socket有两种：TCP Socket和UDP Socket，TCP和UDP是协议，而要确定一个进程得需要三元组，需要IP地址和端口。
...
在Go语言的`net`包中有一个类型`TCPConn`，这个类型可以用来作为客户端和服务器端交互的通道，它有两个主要的函数：
...
还有我们需要知道一个`TCPAddr`类型，它表示一个TCP的地址信息，它的定义如下：
